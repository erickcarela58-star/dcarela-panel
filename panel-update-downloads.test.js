...
test("la vista CURRENT muestra las descargas sin abrir el panel anterior", () => {
  assert.match(shellHtml, /updates-downloads-current\.js\?v=1\.0\.35\.5/);
  assert.match(shellHtml, /updates-downloads-current\.css\?v=1\.0\.35\.5/);
...
test("el manifiesto publica EXE, ZIP y las dos IPA con integridad verificable", () => {
  assert.equal(manifest.web_version, "1.0.35.5");
